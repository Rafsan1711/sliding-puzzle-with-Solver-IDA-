...
class PuzzleState {
  ...
  manhattan() {
    ...
    for(let i=0;i<sz*sz;i++) {
      let v=this.tiles[i]; if(v===0) continue;
      let gi=v-1,gr=(gi/sz)|0,gc=gi%sz; let cr=(i/sz)|0,cc=i%sz;
      d+=Math.abs(cr-gr)+Math.abs(cc-gc);
    } return d;
  }
  misplaced() {
    let sz=this.size,cnt=0; for(let i=0;i<sz*sz-1;i++) if(this.tiles[i]!==i+1) cnt++; return cnt;
  }
  neighbors() {
    let sz=this.size,res=[]; let r=(this.empty/sz)|0,c=this.empty%sz;
...
function ida_star(start,goal,h,limit=1e6,timeout=10000){
  ...
  function dfs(state,g,prev){nodes++;if(nodes>limit)return 1e9;let f=g+h(state);
    if(f>threshold)return f;if(state.isSolved()){found=true;res=path.slice();return -1;}
    let min=1e9;
    for(const n of state.neighbors()){
      if(prev && n.state.key()===prev.key())continue;
      path.push(n.move);
      let t=dfs(n.state,g+1,state);
      if(found)return -1;if(t<min)min=t;
      path.pop();
...
function solve(state,algoList=['ida_star','a_star','bfs','dfs_limited','iddfs','bibfs','dijkstra','greedy_best_first','rbfs','sma_star','dfbnb','bfbnb','hill_climbing','simulated_annealing','beam_search','genetic','tabu_search']) {
  ...
  for(let algo of algoList) {
    ...
    if(moves && moves.length) { result=moves; break; }
  }
  return {result,tried};
}
